...
const pageDescriptions: { [key: string]: string } = {
    ...
    '/anomalies': '실시간으로 감지된 이상 거래를 모니터링하고 관리합니다',
...
export default function Header() {
    ...
    return (
        <header className="bg-[#1e293b] text-white h-16 flex items-center justify-between px-8 fixed top-0 left-64 right-0 z-10 shadow-sm">
            <div>
                {/* Header content if needed, currently empty in dark header design, 
            but the image shows a dark header bar with breadcrumbs or title inside the main area?
            Wait, looking at uploaded_image_0 again.
            The sidebar is dark. The top bar is ALSO dark?
            Let's look closely at uploaded_image_0_1764644103855.png
            Top bar is dark navy #1e293b.
            Left: "Caffeine 관리자" is in the sidebar top.
            Top header bar has "대시보드" breadcrumb? No, the title "대시보드" is in the main content area (white bg).
            The top bar seems to have a user profile on the right.
            Actually, looking at the top right of the image:
            There is a dark bar extending from the sidebar across the top.
            Inside it, on the right: "67% - + 초기화" and user profile "관리자".
            
            Okay, so the layout is:
            Sidebar (Left, full height)
            Header (Top, right of sidebar, dark bg)
            Main Content (Below header, right of sidebar, light bg)
        */}
